...
const createDuffelLink = async (offerId: string) => {
  try {
    ...
    if (result.success && result.link_url) {
      window.location.href = result.link_url; // redirect to Duffel
    } else {
  ...
  <>
    ...
    <div className="flex gap-2 mb-6">
      <button 
        ...
        className={`px-6 py-2 rounded-xl ${journeyType === 'one_way' ? 'bg-sky-500' : 'bg-zinc-800'}`}
      >
        One way
      </button>
      <button 
        onClick={() => setJourneyType('return')} 
        className={`px-6 py-2 rounded-xl ${journeyType === 'return' ? 'bg-sky-500' : 'bg-zinc-800'}`}
      >
        Return
      </button>
    </div>
    ...
    <div className="bg-zinc-900 border border-zinc-700 rounded-3xl p-8 grid grid-cols-1 md:grid-cols-6 gap-4 mb-8">
      ...
      <button 
        ...
        className="bg-sky-500 text-white py-4 rounded-2xl font-bold col-span-1 md:col-span-1"
      >
        SEARCH FLIGHTS
      </button>
    </div>
    ...
    {offers.length > 0 && (
      <div className="space-y-4">
        <h3 className="text-xl font-semibold mb-4">Choose a flight</h3>
        ...
        {offers.map((offer, index) => {
          ...
          return (
            <div key={index} className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <div className="font-semibold text-lg">{airline}</div>
                <div className="text-sm text-zinc-400 mt-1">
                  {slice?.segments?.length > 1 ? `${slice.segments.length - 1} stop` : 'Direct'} • {slice?.duration || ''}
                </div>
                <div className="text-emerald-400 mt-2">
                  {segment ? new Date(segment.departing_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''} 
                  → {segment ? new Date(segment.arriving_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                </div>
              </div>
              ...
              <div className="text-right">
                ...
                <button 
                  onClick={() => createDuffelLink(offer.id)}
                  className="mt-3 bg-emerald-500 hover:bg-emerald-600 text-white px-8 py-3 rounded-xl font-bold"
                >
                  Book with Duffel →
                </button>
              </div>
